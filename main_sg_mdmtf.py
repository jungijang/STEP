...
def main():
    ...
    for test_idx in range(test_num):
        ...
        model_name = 'MDMTF'
        model = MDMTF(3, [num_users, num_items, num_times], K)
        model.to(DEVICE)
        ...
        dim=0, return_inverse=True, return_counts=True)
            ...
            for num_index in tqdm(range(num_times)):
                split_a = torch.split(testa, testa.shape[0]//20)
        ...
        idx_test = np.argpartition(pred_res1, -k)[-k:] 
        sorted_list = idx_test[np.argsort(pred_res1[idx_test])][::-1] 
