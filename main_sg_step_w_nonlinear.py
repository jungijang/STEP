...
from model import SCostCo, SMLP
from neat import SNeAT
from topk_predict import topk_predict, apk_function2, precision_recall2
...
def main():
    ...
    for test_idx in range(test_num):
        ...
        model_name = 'SCostCo'
        model = SCostCo(num_users, num_items, num_times, K, beta)

        # model_name = 'SMLP'
        # model = SMLP(num_users, num_items, num_times, K, beta)        
        ...
        dim=0, return_inverse=True, return_counts=True)
                ...
                loss += l2_lambda * torch.mean(F.softplus(model.angle_weight(i,j,k)))
